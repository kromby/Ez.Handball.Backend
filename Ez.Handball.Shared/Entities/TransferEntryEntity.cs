...
// (e.g. "fantasy-2026-W24"); RowKey = "{reverseTicks}-{playerId}-{userId}" (newest first,
// unique across buy/sell/buy of the same player). Never updated, never deleted.
public sealed class TransferEntryEntity : ITableEntity
{
    public string PartitionKey { get; set; } = string.Empty; // "{flavor}-{ISO-week}"
    public string RowKey { get; set; } = string.Empty;       // "{reverseTicks}-{playerId}-{userId}"
